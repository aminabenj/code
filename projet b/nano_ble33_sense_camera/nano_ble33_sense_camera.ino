...
BLEService monService("19b10000-e8f2-537e-4f6c-d104768a1214");
BLECharacteristic maCaracteristique("19b10000-e8f2-537e-4f6c-d104768a1214", BLERead | BLENotify, 20);
...
void setup()
{
    ...
    if (!BLE.begin()) {
        Serial.println("Démarrage de BLE échoué !");
        while (1);
    }

    BLE.setLocalName("ArduinoBLE");
    BLE.setAdvertisedService(monService);
    monService.addCharacteristic(maCaracteristique);
    BLE.addService(monService);
    maCaracteristique.writeValue("Prêt");
    BLE.advertise();
    Serial.println("Bluetooth activé et prêt à se connecter");
}
...
// Après avoir exécuté run_classifier
float max_value = 0.0;
String max_class = "";
for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
    if (result.classification[ix].value > max_value) {
        max_value = result.classification[ix].value;
        max_class = result.classification[ix].label;
    }
}

// Maintenant, envoyez max_class via BLE
if (max_class.length() > 0) {
    maCaracteristique.writeValue(max_class.c_str());
}
